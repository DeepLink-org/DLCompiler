...
def _ttir_to_linalg(mod, metadata, opt):
    ttir_code = str(mod)
...
def _linalg_to_llir(linalg_ir: str, metadata, opt):
    with tempfile.TemporaryDirectory() as tmpdir:
...
class CPUBackend(BaseBackend):
    ...
    def add_stages(self, stages, options, language):
        ...
        stages["linalg"] = lambda src, metadata: _optimize_linalg(
            _ttir_to_linalg(src, metadata, options)
        )
        stages["llir"] = lambda src, metadata: _optimize_llir(
            _linalg_to_llir(src, metadata, options)
        )
...
def compile_module(launcher_src, kernel_placeholder_name):
    ...
    cpu_verify_dir = os.path.join(cpu_backend_path, "cpu_verify")
