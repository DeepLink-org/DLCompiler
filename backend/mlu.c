...
static inline bool isResultSuccessed(CNresult code, const char *file, int line) {
  if (code == CN_SUCCESS)
    return true;
 
  const char *prefix = "Triton Error [MLU]: ";
  char buffer [33];
  citoa(line,buffer,10);
  const char *str;
...
static void* initCNContext(int device) {
  ...
  MLU_CHECK_AND_RETURN_NULL_ALLOW_THREADS(cnSharedContextAcquire(&pctx, device));
...
static PyObject *getDeviceProperties(PyObject *self, PyObject *args) {
  ...
  MLU_CHECK_AND_RETURN_NULL(cnDeviceGetAttribute(
      &cluster_num, CN_DEVICE_ATTRIBUTE_MAX_CLUSTER_COUNT, pdev));
  MLU_CHECK_AND_RETURN_NULL(cnDeviceGetAttribute(
      &core_num_per_cluster, CN_DEVICE_ATTRIBUTE_MAX_CORE_COUNT_PER_CLUSTER,
      pdev));
  MLU_CHECK_AND_RETURN_NULL(cnDeviceGetAttribute(
      &nram_size, CN_DEVICE_ATTRIBUTE_NEURAL_RAM_SIZE_PER_CORE, pdev));
  MLU_CHECK_AND_RETURN_NULL(cnDeviceGetAttribute(
  ...
  return Py_BuildValue("{s:i, s:i, s:i, s:i, s:i, s:i, s:i, s:i, s:i, s:i, s:i}",
                       ...
                       "core_num_per_cluster", core_num_per_cluster,
                       "max_nram_size", nram_size, "max_shared_mem", sram_size,
                       "cluster_clock_rate", cluster_clock_rate,
                       "memory_bus_width", memory_bus_width,
                       "memory_clock_rate", memory_clock_rate,
                       "isa_version", isa_version,
                       "max_block_task_dim_x", max_block_task_dim_x,
                       "max_block_task_dim_y", max_block_task_dim_y,
                       "max_block_task_dim_z", max_block_task_dim_z);
}
...
static PyObject *loadBinary(PyObject *self, PyObject *args) {
  ...
  printf("name: %s, data size: %d, shared: %d, device: %d", name, data_size, shared, device);
  CNkernel fun;
  CNmodule mod;
  int32_t n_regs = 0;
  int32_t n_spills = 0;
...
static struct PyModuleDef ModuleDef = {PyModuleDef_HEAD_INIT, "mlu_utils",
                                       NULL, // documentation
                                       -1,   // size
                                       ModuleMethods};

PyMODINIT_FUNC PyInit_mlu_utils(void) {
