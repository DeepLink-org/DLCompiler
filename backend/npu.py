...
def _downgrade_mlir_for_legacy_llvm(content: str) -> str:
    content = re.sub(r"(?<=<)([^>]*?)\*(?=x)", lambda m: m.group(1) + "?", content)
    content = re.sub(
        r"(memref\<[^>]*\>)\s+to\s+(tensor\<[^>]*\>)",
        r"\1 // to \2",
        content,
    )
...
def linalg_to_bin_enable_npu_compile_910_95(linalg: str, metadata, opt):
    ...
    if opt.debug or dump_ir:
        dicp_utils._dump_stage_ir(linalg, metadata["hash"], "kernel.dicp.mlir")
    with tempfile.TemporaryDirectory() as tmpdir:
        dicp_path = os.path.join(tmpdir, "kernel.dicp.mlir")
        Path(dicp_path).write_text(linalg)
        ...
        disable_tightly_coupled_buffer_reuse = metadata[
        ...
        if enable_mixed_cv is not None:
            _compile_option_list += [f"--enable-mixed-cv={enable_mixed_cv}"]
        ...
        if enable_drop_unit_dims is not None:
            _compile_option_list += [f"--enable-drop-unit-dims={enable_drop_unit_dims}"]

        enable_flatten = metadata["enable_flatten"]
        if enable_flatten is not None:
            _compile_option_list += [f"--enable-flatten={enable_flatten}"]
        ...
        auto_vectorize_v2_max_fused_ops_num = metadata[
        ...
        hfusion_enable_multiple_consumer_fusion = metadata[
...
def linalg_to_bin_enable_npu_compile_A2_A3(linalg: str, metadata, opt):
    ...
    if _get_bishengir_llvm_version() < 22:
        linalg = _downgrade_mlir_for_legacy_llvm(linalg)
    if opt.debug or dump_ir:
        dicp_utils._dump_stage_ir(linalg, metadata["hash"], "kernel.npuir_input.mlir")
    with tempfile.TemporaryDirectory() as tmpdir:
        dicp_path = os.path.join(tmpdir, "kernel.dicp.mlir")
        Path(dicp_path).write_text(linalg)
        bin_file = os.path.join(tmpdir, "kernel")
