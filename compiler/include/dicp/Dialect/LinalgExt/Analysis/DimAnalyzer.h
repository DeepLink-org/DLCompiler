...
llvm::StringRef toString(DimKind k);
...
class DimAnalyzer {
public:
  explicit DimAnalyzer(const SmallVector<Operation *, 16> &inputOps);
  ...
  /// Runs only the BFS dimension propagation (builds the DSU) without selecting
  /// tiling dims. Use this + getDimRoot()/getDimKind() for cross-unit queries
  /// at sub-stage or stage scope.
  void runAnalysis();

  /// Returns the DSU root ID for a given value's dimension.
  /// Must be called after runAnalysis() or analyzeAndGetTilingDims().
  std::optional<int64_t> getDimRoot(Value v, int64_t dimIdx);

  /// Returns the DimKind for a given value's dimension.
  /// Must be called after runAnalysis() or analyzeAndGetTilingDims().
  DimKind getDimKind(Value v, int64_t dimIdx);

private:
  const SmallVector<Operation *, 16> &inputOps;
  ...
  template <typename ReshapeOp> void processReshapeOp(ReshapeOp op);
