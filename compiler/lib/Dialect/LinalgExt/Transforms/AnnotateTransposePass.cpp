...
bool isPermutedOrHasNonUnitLastStride(MemRefType memRefType) {
  ...
  // 对于 Ascend 来说，如果最后维度 stride !=
  // 1，通常意味着不是连续内存，可能需要隐式转置
...
struct AnnotateTransposePass
    : public mlir::dicp::LinalgExt::impl::AnnotateTransposePassBase<
          AnnotateTransposePass> {
  ...
  void runOnOperation() override {
    ...
    // ==============================================================================
    // 1. 遍历 memref.copy 操作
    //    核心逻辑：检测 Dynamic Subview Copy -> 重写为 Static Full Copy +
    //    Annotation
    // ==============================================================================
    funcOp.walk([&](memref::CopyOp copyOp) {
      ...
      if (srcSubView && dstSubView) {
        ...
        if (baseSourceType && baseTargetType &&
            baseSourceType.hasStaticShape() &&
            baseTargetType.hasStaticShape()) {

          bool isBaseSourcePermuted =
              isPermutedOrHasNonUnitLastStride(baseSourceType);
          // 简化判定：如果不是 Permuted 且 stride 正常，视为 Contiguous
          bool isBaseTargetContiguous =
              !isPermutedOrHasNonUnitLastStride(baseTargetType);
          ...
          if (isBaseSourcePermuted && isBaseTargetContiguous &&
            ...
            llvm::outs() << "  [REWRITE_MATCH] Found Dynamic Subview Copy "
            ...
            if (auto sourceDefOp = target.getDefiningOp()) {
              if (auto subviewOp = dyn_cast<memref::SubViewOp>(sourceDefOp)) {
                ...
                for (auto user : parentMemRef.getUsers()) {
                  if (auto toTensorOp =
                          dyn_cast<bufferization::ToTensorOp>(user)) {
                    bool exists = false;
                    for (auto op : toTensorOpsToMark)
                      if (op == toTensorOp)
                        exists = true;
    ...
    funcOp.walk([&](bufferization::ToTensorOp toTensorOp) {
      // 如果已经在列表中，跳过
      for (auto existing : toTensorOpsToMark) {
        if (existing == toTensorOp)
          return;
      }

      Value sourceMemRef = toTensorOp.getOperand();
      bool hasNonStandardStride =
          checkValueOriginHasNonStandardStride(sourceMemRef);
      ...
      if (shouldMark || hasNonStandardStride) {
        toTensorOpsToMark.push_back(toTensorOp);
        llvm::outs() << "[TO_TENSOR_CHECK] Found permuted/strided origin: "
                     << toTensorOp << "\n";
      }
    });
    ...
    for (auto toTensorOp : toTensorOpsToMark) {
      // 双重检查：防止重复添加 MarkOp (虽然 OpBuilder 会创建新的
      // Op，但逻辑上我们不希望冗余) 简单检查该 Value 是否已经被 MarkOp 使用
      bool alreadyMarked = false;
      // 注意：annotation::MarkOp 通常不直接作为 User 挂在 Value
      // 上，而是作为一个独立的 Op 存在。 为了稳妥，这里我们假设 list
      ...
      auto markOp = builder.create<annotation::MarkOp>(toTensorOp->getLoc(),
