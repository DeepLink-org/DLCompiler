  ...
  for (auto it : llvm::zip(initOperands, regionIterArgs)) {
    ...
    for (Operation *readOp : candidates) {
      ...
      if (!readInterface || !writeInterface) {
        LDBG("      Reject: one of the ops does not implement "
             "OffsetSizeAndStrideOpInterface.");
        continue;
      }

      FailureOr<bool> equivalent = proveSlicesEquivalent(
          readOp->getContext(), readInterface.getMixedOffsets(),
          readInterface.getMixedSizes(), readInterface.getMixedStrides(),
          writeInterface.getMixedOffsets(), writeInterface.getMixedSizes(),
          writeInterface.getMixedStrides());
      if (failed(equivalent)) {
        LDBG("      Reject: could not prove subset equivalence.");
        continue;
      }
...
transform::ExtendedFuseIntoContainingOp::fuseIntoOneContaining(
  ...
  while (!remainingProducers.empty()) {
    ...
    // Cross-user producers are allowed to remain unfused. They are kept to
    // preserve the original shared SSA value for uses outside the current
    // tiling unit when no legal in-loop fusion path is available.
    if (producerOp->hasAttr(kCrossTillUnitAttr)) {
      LDBG("Skipping cross-user producer with no legal fusion path: "
           << *producerOp);
      remainingProducers.erase(remainingProducers.begin() + producerIndex);
      continue;
    }
