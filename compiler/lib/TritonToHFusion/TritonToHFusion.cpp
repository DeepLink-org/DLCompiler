...
namespace {
struct TritonModToHFusionConversion : OpRewritePattern<triton::dicp::ModOp> {
  using OpRewritePattern<triton::dicp::ModOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(triton::dicp::ModOp op,
...
struct TritonConv1dToHFusionConversion
    : OpRewritePattern<triton::dicp::Conv1dOp> {
  using OpRewritePattern<triton::dicp::Conv1dOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(triton::dicp::Conv1dOp op,
                                PatternRewriter &rewriter) const final {
    auto loc = op.getLoc();
