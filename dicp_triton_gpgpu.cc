...
#include "dicp/Dialect/GPGPU/Transforms/Passes.h"
  ...
  builder_cls
      ...
      .def("make_nv_mma_encoding_attr",
           [](TritonOpBuilder &self, Value opndA, Value opndAcc,
             ...
             unsigned numWarps =
                 ttg::maybeLookupNumWarps(parentBlock->getParentOp())
                     .value_or(moduleNumWarps);
             if (auto partitions = dyn_cast<ttg::WarpSpecializePartitionsOp>(
                     parentBlock->getParentOp())) {
               unsigned idx = parentBlock->getParent()->getRegionNumber();
               numWarps = partitions.getParentOp().getPartitionNumWarps()[idx];
             }
      ...
      .def("create_make_tensor_descriptor",
           [](TritonOpBuilder &self, Value &base, std::vector<Value> &shape,
             ...
             return self.create<mlir::triton::MakeTensorDescOp>(
