...
unary_op_impl(void (*intrin)(__ubuf__ T *, __ubuf__ T *, uint64_t, uint16_t,
...
binary_op_impl(void (*intrin)(__ubuf__ T *, __ubuf__ T *, __ubuf__ T *,
  ...
  // 双目: (dst, src0, src1, repeat, dst_bs, src0_bs, src1_bs, dst_rs, src0_rs,
  // src1_rs)
...
// --- vexp: y = exp(x) ---
__aiv__ __attribute__((always_inline)) void
_mlir_ciface_custom_vexp_fp32(memref_t<__ubuf__ float, 1> *src,
...
// --- vdiv: z = x / y ---
__aiv__ __attribute__((always_inline)) void
_mlir_ciface_custom_vdiv_fp32(memref_t<__ubuf__ float, 1> *src0,
...
// --- vsub: z = x - y ---
__aiv__ __attribute__((always_inline)) void
_mlir_ciface_custom_vsub_fp32(memref_t<__ubuf__ float, 1> *src0,
                              memref_t<__ubuf__ float, 1> *src1,
                              memref_t<__ubuf__ float, 1> *dst) {
  auto s0 = src0->aligned + src0->offset;
  auto s1 = src1->aligned + src1->offset;
...
// --- vmul: z = x * y ---
__aiv__ __attribute__((always_inline)) void
_mlir_ciface_custom_vmul_fp32(memref_t<__ubuf__ float, 1> *src0,
                              memref_t<__ubuf__ float, 1> *src1,
                              memref_t<__ubuf__ float, 1> *dst) {
  auto s0 = src0->aligned + src0->offset;
  auto s1 = src1->aligned + src1->offset;
