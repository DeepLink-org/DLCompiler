...
def mangle_ty(ty):
    ...
    from ...buffer.core import buffer_type

    if isinstance(ty, buffer_type):
...
def _handle_core_mode_attr(builder, core_mode):
    ...
    return {
        builder.get_t_core_type_attr_name(): (
            builder.get_t_core_type_cube_attr()
            if core_mode == "cube"
...
def _build_mlir_attrs_from_scope_attrs(builder, scope_attrs):
    ...
    for k, v in scope_attrs.items():
        if k == "core_mode":
            ...
            if v:
                mlir_attrs["hivm.disable_auto_sync"] = _py_value_to_mlir_attr(
                    builder, v
                )
        else:
...
def _verify_loop_carried_variable(
    _is_triton_value, _is_triton_tensor, name, loop_val, live_val
):
...
def handle_scope_with(generator, node):
    ...
    with enter_sub_region(generator) as sr:
        ...
        entry_block = generator.builder.create_block_with_parent(
            scope_op.get_region(0), []
        )
    ...
    for i, name in enumerate(names):
        generator.set_value(
            name,
            _reconstruct_value_from_ir(language, scope_op.get_result(i), ret_types[i]),
        )
    return None
