...
from triton.backends.dicp_triton.utils import get_ascend_arch_from_env
import triton


#  TODO :triton-ascend 拆分为 extension/math_ops.py、extension/vec_ops.py，DLCompiler 直接放在 libdevice.py 中

# ---------------------------------------------------------------------------
# Pure extern functions (bitcode __hmf_* symbols)
# ---------------------------------------------------------------------------
...
def tanh(arg0, _semantic=None):
    ...
    res = core.extern_elementwise(
        "", "", [arg0], dispatch, is_pure=True, _semantic=_semantic
    )
    if original_dtype == core.dtype("bf16"):
...
def atan2(arg0, arg1, _semantic=None):
    if arg0.dtype == core.dtype("bf16") or arg1.dtype == core.dtype("bf16"):
        core.static_print(
            "extern libdevice.atan2 for dtype bf16 is unsupported for now."
        )
...
# ---------------------------------------------------------------------------
# IR builder math implementations (SIMD path)
# ---------------------------------------------------------------------------
...
def acos(arg0: core.tensor, _semantic=None):
    ...
    acos_mid_signed = _semantic.where(
        is_neg_mid, _semantic.sub(3.1415926536, acos_mid, True), acos_mid
    )
    is_center = _semantic.less_than(abs_x, 0.6)
    return _semantic.where(is_center, acos_center, acos_mid_signed)
...
# Derived from the Cephes Math Library release 2.8: June, 2000
# https://netlib.org/cephes/
@core.builtin
@math._check_dtype(dtypes=["fp16", "fp32"])
@math._add_math_2arg_docstr(
...
def cyl_bessel_i0(arg0: core.tensor, _semantic=None):
    ...
    half_exp = _semantic.mul(
        core.tensor(_semantic.builder.create_exp(abs_x.handle), abs_x.type), 0.5, True
    )
    ...
    res_b = _semantic.fdiv(
        ...
        core.tensor(_semantic.builder.create_sqrt(abs_x.handle), abs_x.type),
        True,
    )
...
def erfinv(arg0, _semantic=None):
    ...
    inv_sqrt_pi_times_2 = _semantic.full(arg0.shape, 1.128379167, arg0_scalar_ty).handle
    ...
    numerator_low_range = _semantic.full(
        arg0.shape, coeff_low_numerator[0], arg0_scalar_ty
    ).handle
    ...
    low_res = _semantic.builder.create_fmul(
        arg0.handle,
        _semantic.builder.create_fdiv(numerator_low_range, denominator_low_range),
    )

    arg0_erf_trans = _semantic.builder.create_sqrt(
    ...
    numerator_high_range = _semantic.full(
        arg0.shape, coeff_high_numerator[0], arg0_scalar_ty
    ).handle
    for i in range(1, len(coeff_high_numerator)):
        numerator_high_range = _semantic.builder.create_fma(
            numerator_high_range,
            arg0_erf_trans,
            _semantic.full(arg0.shape, coeff_high_numerator[i], arg0_scalar_ty).handle,
        )

    denominator_high_range = _semantic.full(
        arg0.shape, coeff_high_denominator[0], arg0_scalar_ty
    ).handle
    for i in range(1, len(coeff_high_denominator)):
        denominator_high_range = _semantic.builder.create_fma(
            denominator_high_range,
            arg0_erf_trans,
            _semantic.full(
                arg0.shape, coeff_high_denominator[i], arg0_scalar_ty
            ).handle,
        )

    high_res = _semantic.builder.create_fdiv(
        numerator_high_range, denominator_high_range
    )
    high_res = _semantic.mul(
        ...
        core.tensor(high_res, arg0.type),
        True,
    ).handle
...
def gamma(arg0, _semantic=None):
    ...
    sqrt_2pi_tensor = _semantic.full(arg0.shape, 2.506628275, arg0_scalar_ty).handle
    lanczos_coeff = [
        676.5203681218851,
        -1259.1392167224028,
        771.32342877765313,
    ...
    condition = _semantic.less_than(arg0, 0.5)
    ...
    for i in range(0, len(lanczos_coeff)):
        x = _semantic.add(
            ...
            _semantic.fdiv(
                ...
                _semantic.add(reflect_arg0, i, True),
                True,
            ),
            True,
        )
    ...
    gamma_res_reflect = _semantic.builder.create_fdiv(
        ...
        _semantic.builder.create_sin(
            _semantic.builder.create_fmul(pi_tensor, arg0.handle)
        ),
    )
    ...
    res = _semantic.where(
        condition, gamma_res_reflect, core.tensor(gamma_res, arg0.type)
    )
    ...
    return _semantic.where(
        is_pos_inf_input,
        pos_inf_tensor,
        _semantic.where(is_neg_inf_input, neg_inf_tensor, res),
    )
...
def nearbyint(arg0: core.tensor, _semantic=None):
    ...
    positive_result = core.tensor(
        _semantic.builder.create_floor(positive_adjust.handle), arg0.type
    )
    negative_result = core.tensor(
        _semantic.builder.create_ceil(negative_adjust.handle), arg0.type
    )
    zero = _semantic.full(arg0.shape, 0.0, arg0.type.scalar)
    ...
    abs_fractional = core.tensor(
        _semantic.builder.create_fabs(fractional.handle), fractional.type
    )
    ...
    half_floor = core.tensor(
        _semantic.builder.create_floor(half_value.handle), half_value.type
    )
    double_half = _semantic.mul(half_floor, two, True)
    is_even = _semantic.equal(basic_round, double_half)

    adjustment = _semantic.where(
        is_positive,
        _semantic.full(arg0.shape, -1.0, arg0.type.scalar),
        _semantic.full(arg0.shape, 1.0, arg0.type.scalar),
    )
    banker_result = _semantic.where(
        is_even, basic_round, _semantic.add(basic_round, adjustment, True)
    )
...
def asin(arg0: core.tensor, _semantic=None):
    arg0 = _semantic.to_tensor(arg0)
    half_pi = _semantic.full(arg0.shape, 1.5707963267948966, arg0.type.scalar)
...
def copysign(arg0: core.tensor, arg1: core.tensor, _semantic=None):
    ...
    neg_magnitude = _semantic.mul(
        magnitude, _semantic.full(magnitude.shape, -1.0, magnitude.type.scalar), True
    )
...
if get_ascend_arch_from_env() == "Ascend910_9589":
    ...
    def rint(arg0: core.tensor, _semantic=None):
        ...
        is_half = _semantic.less_than(
            math.abs(_semantic.sub(fractional, half, True), _semantic=_semantic), eps
        )
        floor_int = (
            floor_x.to(core.int32, _semantic=_semantic)
            if hasattr(floor_x, "to")
            else _semantic.cast(floor_x, core.int32)
        )
        two_i32 = _semantic.full(arg0.shape, 2, core.int32)
        is_even = _semantic.equal(
            _semantic.mod(floor_int, two_i32), _semantic.full(arg0.shape, 0, core.int32)
        )
        zero = _semantic.full(arg0.shape, 0.0, arg0.type.scalar)
        ...
        half_round = _semantic.where(
            is_even, floor_x, _semantic.add(floor_x, 1.0, True)
        )
        return _semantic.where(is_half, half_round, normal_round)


@core.builtin
def flip(arg0, dim=-1, _semantic=None):
    arg0 = _semantic.to_tensor(arg0)
    shape = arg0.shape
    rank = len(shape)
    if dim < 0:
        dim = dim + rank
    if not (0 <= dim < rank):
        raise ValueError(f"flip got invalid dim={dim} for shape {tuple(shape)}")
    flipped = _semantic.builder.create_flip(arg0.handle, dim)
    return core.tensor(flipped, arg0.type)


# ---------------------------------------------------------------------------
# JIT-style functions using libdevice primitives
# ---------------------------------------------------------------------------
...
def isfinited(x):
    ...
    core.static_assert(
        not _is_int8_type, f"Expected dtype fp16/fp32/bf16, but got int8 or int1"
    )
    _is_floating_type: core.constexpr = x.dtype.is_floating()
    core.static_assert(
        _is_floating_type == True,
        f"Expected dtype fp16/fp32/bf16, but got {core.constexpr(x.dtype)}",
    )
    nan_mask = isnan(x)
    inf_mask = isinf(x)
...
def finitef(x):
    _is_int8_type: core.constexpr = x.dtype.is_int8()
    core.static_assert(
        not _is_int8_type, f"finitef only supports float32, but got int8 or int1"
    )
    core.static_assert(
        x.dtype == core.float32,
        f"finitef only supports float32, but got {core.constexpr(x.dtype)}",
    )
    nan_mask = isnan(x)
    inf_mask = isinf(x)
    return (~nan_mask & ~inf_mask).to(core.int1)
