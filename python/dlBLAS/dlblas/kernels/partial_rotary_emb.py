...
@triton.jit
def _partial_rotary_emb_kernel(
    q,
    k_pe,
    kv,
    cos,
...
class PartialRotaryEmb(torch.autograd.Function):
    ...
    def forward(ctx: torch.Any, q, k_pe, kv, cos, sin):
        ...
        out_kv = kv.new_empty(bsz, q_len, 2, num_heads, q_head_dim)
        with torch.cuda.device(q.device):
            grid = lambda META: (bsz, triton.cdiv(q_len, META["BLOCK_SEQ"]), num_heads)
            _partial_rotary_emb_kernel[grid](
                q,
                k_pe,
                kv,
                cos,
            ...
            #     _partial_rotary_emb_kernel.best_config,
            # )
        return q, out_kv
...
def bench_fn(q, k_pe, kv, cos, sin):
    ...
    ms = triton.testing.do_bench(fn, warmup=5, rep=5)
    return ms
