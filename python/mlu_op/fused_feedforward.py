...
x = torch.randn([1, 16, 4096], dtype=torch.float16, device="mlu")
# weights tends to be very small values
rms_w = torch.randn([4096], dtype=torch.float16, device="mlu") * 0.2
w1_w = torch.randn([11008, 4096], dtype=torch.float16, device="mlu") * 0.2
w3_w = torch.randn([11008, 4096], dtype=torch.float16, device="mlu") * 0.2
...
# BENCH
print("rms matmul silu mul triton", triton.testing.do_bench(lambda: kernel_ff(x=x, w1=w1_w, w3=w3_w, rms_w=rms_w),warmup=100, rep=100 ))
print("rms matmul silu mul pytorch", triton.testing.do_bench(lambda: ff_pytorch(x=x, w1=w1_w, w3=w3_w, rms_w=rms_w), warmup=100, rep=100))