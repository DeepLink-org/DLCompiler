...
@triton.autotune(                                                                                                                                                                                               
    configs=[
        triton.Config(
            { 'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 64, 'BLOCK_SIZE_K': 256, 'GROUP_SIZE_M': 8 },
            num_stages=0, num_warps=1),
        triton.Config(
            { 'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 64, 'BLOCK_SIZE_K': 256, 'GROUP_SIZE_M': 8 },
            num_stages=0, num_warps=1),
        triton.Config(
        ...
        triton.Config(
            ...
            num_stages=0, num_warps=1),
        triton.Config(
            { 'BLOCK_SIZE_M': 160, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 160, 'GROUP_SIZE_M': 8 },
            num_stages=0, num_warps=1),
        triton.Config(
...
def matmul_kernel(
    ...
    offs_am = tl.max_contiguous(
        tl.multiple_of((pid_m * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)) % M,
                       BLOCK_SIZE_M), BLOCK_SIZE_M)
    offs_bn = tl.max_contiguous(
        tl.multiple_of((pid_n * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)) % N,
...
torch.manual_seed(0)
wl = 1024
a = torch.randn((wl, wl), device='mlu', dtype=torch.float16)
b = torch.randn((wl, wl), device='mlu', dtype=torch.float16)
...
def benchmark(M, N, K, provider):
    ...
    if provider == 'torch':
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: torch.matmul(a, b), quantiles=quantiles, warmup=100,rep=100)
    if provider == 'triton':
