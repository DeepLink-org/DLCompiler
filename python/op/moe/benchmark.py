...
from topk_gating import TopKGatingFunc, _capacity, gumbel_rsample
...
def fused_topkgating(
    ...
    # print(f"locations:{locations_s.shape}")
...
def test():
    device_ = torch.device('cuda:5')
    torch.cuda.set_device(device_)
    # k, SeqLen, NumberExperts = 3, 4, 8
    k, SeqLen, NumberExperts = 6, 1024, 64
    ...
    output1_torch, output2_torch, output3_torch, output4_torch = model_torch(logits_torch, k)
    output1_triton, output2_triton, output3_triton, output4_triton = model_triton(logits_triton, k)
