...
void init_gluon_ir(py::module &&m) {
  ...
  py::class_<GluonOpBuilder, TritonOpBuilder>(
      ...
      .def("create_extract_tensor",
           [](GluonOpBuilder &self, Type resultTy, Value source,
              std::vector<int64_t> &ctaIdx,
              std::vector<int64_t> &elemIdx) -> Value {
             return self
                 .create<ttg::ExtractTensorOp>(resultTy, source, ctaIdx,
                                               elemIdx)
                 .getResult();
           })
      .def("create_insert_tensor",
           [](GluonOpBuilder &self, Type resultTy, Value inserted,
              Value insert, std::vector<int64_t> &ctaIdx,
              std::vector<int64_t> &elemIdx) -> Value {
             return self
                 .create<ttg::InsertTensorOp>(resultTy, inserted, insert,
                                              ctaIdx, elemIdx)
                 .getResult();
           })
