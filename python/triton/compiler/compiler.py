...
def compile(src, target=None, options=None, _env_vars=None):
    ...
    for ext, compile_ir in list(stages.items())[first_stage:]:
        ...
        if (not store_only_binary) or (ext in ("cubin", "hsaco", "json") or
                                       (knobs.metax.use_maca and ext == "mcfatbin")):
...
class CompiledKernel:
    ...
    def _init_handles(self):
        ...
        # MACA: skip max_threads check
        if driver.active.get_current_target().backend != "maca" and \
           self.metadata.num_warps * warp_size > self.n_max_threads:
