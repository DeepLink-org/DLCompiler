...
# Autotune the forward block geometry while keeping the requested pipeline
# configuration fixed for every candidate.
configs = [
    triton.Config(
        {'BLOCK_M': BM, 'BLOCK_N': BN, 'pipeline': 'basic'},
        num_stages=2,
        num_warps=w,
    )
    for BM in [64, 128]
...
bwd_configs = [
    ...
    for BLOCK_M1, BLOCK_N1, BLOCK_N2 in [
        (32, 32, 32),
        (32, 64, 32),
    ]
...
@triton.autotune(bwd_configs, key=["N_CTX", "HEAD_DIM"])
@triton.jit
...
@pytest.mark.parametrize("Z, H, N_CTX, HEAD_DIM", [(1, 32, 1024, 128)])
...
# Benchmark the requested fixed shape.
configs = []
for mode in ["fwd", "bwd"]:
    for causal in [True, False]:
        ...
        configs.append(
            triton.testing.Benchmark(
                x_names=["N_CTX"],
                x_vals=[512, 1024, 2048, 4096, 10240, 102400],
                line_arg="provider",
