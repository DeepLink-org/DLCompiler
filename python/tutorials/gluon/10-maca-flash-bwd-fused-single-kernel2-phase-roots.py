...
def flash_bwd_fused_kernel(
    ...
    Phase-local, role-shaped allocations expose the shared layout contracts;
    the compiler reuses storage after each phase's final descriptor use.
    """
    ...
    # Each phase below allocates ordinary, role-shaped shared buffers. Their
    # non-overlapping lifetimes let the shared allocator reuse the same 64 KiB
    # without exposing a private descriptor reinterpret in the main program.
    ...
    for fragment in range(0, BLOCK_N // N_FRAGMENT):
        ...
        prologue_dn_slots = gl.local_alloc(
            k_ptr.dtype.element_ty,
            [BLOCK_D, BLOCK_N],
            num_buffers=2,
            layout=tn_shared_layout,
        )
        k_tn_smem = prologue_dn_slots.index(0)
        v_tn_smem = prologue_dn_slots.index(1)
        vt_ptrs = (
            v_head
        ...
        loop_tile_slots = gl.local_alloc(
            q_ptr.dtype.element_ty,
            [BLOCK_M, BLOCK_D],
            num_buffers=3,
            layout=epoch_tile_layout,
        )
        q_smem = loop_tile_slots.index(0)
        do_smem = loop_tile_slots.index(1)
        ds_direct_smem = loop_tile_slots.index(2)

        loop_async_slots = gl.local_alloc(
            q_ptr.dtype.element_ty,
            [BLOCK_M, BLOCK_D],
            num_buffers=3,
            layout=epoch_raw_layout,
        )
        do_raw_smem = loop_async_slots.index(2)

        loop_transposed_slots = gl.local_alloc(
            q_ptr.dtype.element_ty,
            [BLOCK_N, BLOCK_M],
            num_buffers=2,
            layout=transposed_stage_layout,
        )
        ds_transposed_smem = loop_transposed_slots.index(0)
        p_transposed_smem = loop_transposed_slots.index(1)
        # Dot-facing [N,M] descriptors stay direct; only their [M,N] writers
        # use a public coordinate permutation.
        ...
        initial_q_raw_smem = loop_async_slots.index(initial_m_block % 2)
        ...
        for m_iteration in range(0, num_m_blocks):
            ...
            q_raw_smem = loop_async_slots.index(m_block % 2)
            ...
            if m_block > 0:
                next_q_raw_smem = loop_async_slots.index((m_block - 1) % 2)
                gl.metax.async_copy_global_to_shared(
                    next_q_raw_smem,
                    next_q_ptrs,
                    mask=next_q_mask,
        ...
        gl.metax.barrier_shared()
        epilogue_nd_slots = gl.local_alloc(
            dk_ptr.dtype.element_ty,
            [BLOCK_N, BLOCK_D],
            num_buffers=2,
        )
        dk_smem = epilogue_nd_slots.index(0)
        dv_smem = epilogue_nd_slots.index(1)
        dk_smem.store(dk_element)
        dv_smem.store(dv_element)
        gl.metax.barrier_shared()
        ...
        dk_out = dk_smem.load(intrinsic=True, is_constant_offs=True)
        ...
        dv_out = dv_smem.load(intrinsic=True, is_constant_offs=True)
