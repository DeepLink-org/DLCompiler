...
def matmul_kernel(a_ptr, b_ptr, c_ptr, M, N, K, stride_am, stride_bk, stride_cm, BLOCK_M: gl.constexpr,
    ...
    a_smem.index(0).store(a_tile)
    b_smem.index(0).store(b_tile)
    ...
    for _ in range(0, num_k_tiles):
        ...
        # Do not carry memdesc views through scf.for.  This is the same
        # post-inline boundary TLX relies on: the scalar stage index is the
        # loop-carried value and each iteration recreates the Shared view.
        loop_a_stage = a_smem.index(read_stage)
        loop_b_stage = b_smem.index(read_stage)
        a_k0 = loop_a_stage.slice(0, 16, dim=1).load()
        b_k0 = loop_b_stage.slice(0, 16, dim=0).load()
        loop_acc = gl.dot(a_k0, b_k0, loop_acc, input_precision="tf32")

        a_k1 = loop_a_stage.slice(16, 16, dim=1).load()
        b_k1 = loop_b_stage.slice(16, 16, dim=0).load()
        loop_acc = gl.dot(a_k1, b_k1, loop_acc, input_precision="tf32")

        next_a_stage = a_smem.index(next_stage)
        next_b_stage = b_smem.index(next_stage)
...
def matmul(a, b, c, BLOCK_M=128, BLOCK_N=128, BLOCK_K=32, GROUP_SIZE_M=8):
    M, K = a.shape
    Kb, N = b.shape
    assert K == Kb
