...
def matmul_kernel(a_ptr, b_ptr, c_ptr, M, N, K, stride_am, stride_ak, stride_bk, stride_bn, stride_cm, stride_cn,
    ...
    a_ptrs0 = gl.slice(a_ptrs, [32, BLOCK_K], [0 * 32, 0])
    a_mask0 = gl.slice(a_load_mask, [32, BLOCK_K], [0 * 32, 0])
    gl.metax.async_copy_global_to_shared(a_smem.index(0), a_ptrs0, mask=a_mask0, other=0.0)
    b_ptrs0 = gl.slice(b_ptrs, [32, BLOCK_N], [0 * 32, 0])
    b_mask0 = gl.slice(b_load_mask, [32, BLOCK_N], [0 * 32, 0])
    gl.metax.async_copy_global_to_shared(b_smem.index(0), b_ptrs0, mask=b_mask0, other=0.0)

    a_ptrs1 = gl.slice(a_ptrs, [32, BLOCK_K], [1 * 32, 0])
    a_mask1 = gl.slice(a_load_mask, [32, BLOCK_K], [1 * 32, 0])
    gl.metax.async_copy_global_to_shared(a_smem.index(1), a_ptrs1, mask=a_mask1, other=0.0)
    b_ptrs1 = gl.slice(b_ptrs, [32, BLOCK_N], [1 * 32, 0])
    b_mask1 = gl.slice(b_load_mask, [32, BLOCK_N], [1 * 32, 0])
    gl.metax.async_copy_global_to_shared(b_smem.index(1), b_ptrs1, mask=b_mask1, other=0.0)

    a_ptrs2 = gl.slice(a_ptrs, [32, BLOCK_K], [2 * 32, 0])
    a_mask2 = gl.slice(a_load_mask, [32, BLOCK_K], [2 * 32, 0])
    gl.metax.async_copy_global_to_shared(a_smem.index(2), a_ptrs2, mask=a_mask2, other=0.0)
    b_ptrs2 = gl.slice(b_ptrs, [32, BLOCK_N], [2 * 32, 0])
    b_mask2 = gl.slice(b_load_mask, [32, BLOCK_N], [2 * 32, 0])
    gl.metax.async_copy_global_to_shared(b_smem.index(2), b_ptrs2, mask=b_mask2, other=0.0)

    a_ptrs3 = gl.slice(a_ptrs, [32, BLOCK_K], [3 * 32, 0])
    a_mask3 = gl.slice(a_load_mask, [32, BLOCK_K], [3 * 32, 0])
    gl.metax.async_copy_global_to_shared(a_smem.index(3), a_ptrs3, mask=a_mask3, other=0.0)
    b_ptrs3 = gl.slice(b_ptrs, [32, BLOCK_N], [3 * 32, 0])
    b_mask3 = gl.slice(b_load_mask, [32, BLOCK_N], [3 * 32, 0])
    gl.metax.async_copy_global_to_shared(b_smem.index(3), b_ptrs3, mask=b_mask3, other=0.0)
    gl.metax.gvm_arrive(12)
    gl.metax.barrier()

    a0 = a_smem.index(0).load()
    b0_raw = b_smem.index(0).load()
    b0 = gl.metax.bsm_perm(b0_raw)
    gl.metax.gvm_arrive(8)
    gl.metax.barrier()
    a1 = a_smem.index(1).load()
    b1_raw = b_smem.index(1).load()
    b1 = gl.metax.bsm_perm(b1_raw)
    ...
    for k in range(0, num_k_tiles):
        ...
        c0 = gl.slice(acc, [32, BLOCK_N], [0 * 32, 0])
        c1 = gl.slice(acc, [32, BLOCK_N], [1 * 32, 0])
        c2 = gl.slice(acc, [32, BLOCK_N], [2 * 32, 0])
        c3 = gl.slice(acc, [32, BLOCK_N], [3 * 32, 0])

        a_next_offsets = a_offsets
        b_next_offsets = b_offsets

        a_off0 = gl.slice(a_offsets, [32, BLOCK_K], [0 * 32, 0])
        a_inc0 = gl.slice(a_step, [32, BLOCK_K], [0 * 32, 0])
        a_off0 = a_off0 + a_inc0
        a_next_offsets = gl.slice_update(a_next_offsets, a_off0, [0 * 32, 0 * 32])
        a_next_ptrs0 = gl.slice(a_ptr + a_next_offsets, [32, BLOCK_K], [0 * 32, 0])
        a_next_mask0 = gl.slice(next_a_mask, [32, BLOCK_K], [0 * 32, 0])
        gl.metax.async_copy_global_to_shared(a_smem.index(0), a_next_ptrs0, mask=a_next_mask0, other=0.0,
)

        b_off0 = gl.slice(b_offsets, [32, BLOCK_N], [0 * 32, 0])
        b_inc0 = gl.slice(b_step, [32, BLOCK_N], [0 * 32, 0])
        b_off0 = b_off0 + b_inc0
        b_next_offsets = gl.slice_update(b_next_offsets, b_off0, [0 * 32, 0 * 32])
        b_next_ptrs0 = gl.slice(b_ptr + b_next_offsets, [32, BLOCK_N], [0 * 32, 0])
        b_next_mask0 = gl.slice(next_b_mask, [32, BLOCK_N], [0 * 32, 0])
        gl.metax.async_copy_global_to_shared(b_smem.index(0), b_next_ptrs0, mask=b_next_mask0, other=0.0,
)

        a0k0 = gl.slice(a0, [32, 32], [0 * 32, 0 * 32])
        c0 = gl.dot(a0k0, b0, c0, input_precision="tf32")
        gl.metax.iglp(config_2=2, config_5=1, config_6=2)
        gl.metax.gvm_arrive(8)
        gl.metax.barrier_shared()

        b2_raw = b_smem.index(2).load()
        a2 = a_smem.index(2).load()

        a_off1 = gl.slice(a_offsets, [32, BLOCK_K], [1 * 32, 0])
        a_inc1 = gl.slice(a_step, [32, BLOCK_K], [1 * 32, 0])
        a_off1 = a_off1 + a_inc1
        a_next_offsets = gl.slice_update(a_next_offsets, a_off1, [1 * 32, 0 * 32])
        a_next_ptrs1 = gl.slice(a_ptr + a_next_offsets, [32, BLOCK_K], [1 * 32, 0])
        a_next_mask1 = gl.slice(next_a_mask, [32, BLOCK_K], [1 * 32, 0])
        gl.metax.async_copy_global_to_shared(a_smem.index(1), a_next_ptrs1, mask=a_next_mask1, other=0.0,
)

        b_off1 = gl.slice(b_offsets, [32, BLOCK_N], [1 * 32, 0])
        b_inc1 = gl.slice(b_step, [32, BLOCK_N], [1 * 32, 0])
        b_off1 = b_off1 + b_inc1
        b_next_offsets = gl.slice_update(b_next_offsets, b_off1, [1 * 32, 0 * 32])
        b_next_ptrs1 = gl.slice(b_ptr + b_next_offsets, [32, BLOCK_N], [1 * 32, 0])
        b_next_mask1 = gl.slice(next_b_mask, [32, BLOCK_N], [1 * 32, 0])
        gl.metax.async_copy_global_to_shared(b_smem.index(1), b_next_ptrs1, mask=b_next_mask1, other=0.0,
)

        a1k0 = gl.slice(a1, [32, 32], [0 * 32, 0 * 32])
        c1 = gl.dot(a1k0, b0, c1, input_precision="tf32")
        a0k1 = gl.slice(a0, [32, 32], [0 * 32, 1 * 32])
        c0 = gl.dot(a0k1, b1, c0, input_precision="tf32")
        a1k1 = gl.slice(a1, [32, 32], [0 * 32, 1 * 32])
        ...
        b3_raw = b_smem.index(3).load()

        b_off2 = gl.slice(b_offsets, [32, BLOCK_N], [2 * 32, 0])
        b_inc2 = gl.slice(b_step, [32, BLOCK_N], [2 * 32, 0])
        b_off2 = b_off2 + b_inc2
        b_next_offsets = gl.slice_update(b_next_offsets, b_off2, [2 * 32, 0 * 32])
        b_next_ptrs2 = gl.slice(b_ptr + b_next_offsets, [32, BLOCK_N], [2 * 32, 0])
        b_next_mask2 = gl.slice(next_b_mask, [32, BLOCK_N], [2 * 32, 0])
        gl.metax.async_copy_global_to_shared(b_smem.index(2), b_next_ptrs2, mask=b_next_mask2, other=0.0,
)

        a_off2 = gl.slice(a_offsets, [32, BLOCK_K], [2 * 32, 0])
        a_inc2 = gl.slice(a_step, [32, BLOCK_K], [2 * 32, 0])
        a_off2 = a_off2 + a_inc2
        a_next_offsets = gl.slice_update(a_next_offsets, a_off2, [2 * 32, 0 * 32])
        a_next_ptrs2 = gl.slice(a_ptr + a_next_offsets, [32, BLOCK_K], [2 * 32, 0])
        a_next_mask2 = gl.slice(next_a_mask, [32, BLOCK_K], [2 * 32, 0])
        gl.metax.async_copy_global_to_shared(a_smem.index(2), a_next_ptrs2, mask=a_next_mask2, other=0.0,
)

        a2k0 = gl.slice(a2, [32, 32], [0 * 32, 0 * 32])
        c2 = gl.dot(a2k0, b0, c2, input_precision="tf32")
        b2 = gl.metax.bsm_perm(b2_raw)
        a0k2 = gl.slice(a0, [32, 32], [0 * 32, 2 * 32])
        c0 = gl.dot(a0k2, b2, c0, input_precision="tf32")
        a2k1 = gl.slice(a2, [32, 32], [0 * 32, 1 * 32])
        c2 = gl.dot(a2k1, b1, c2, input_precision="tf32")
        a1k2 = gl.slice(a1, [32, 32], [0 * 32, 2 * 32])
        c1 = gl.dot(a1k2, b2, c1, input_precision="tf32")
        a2k2 = gl.slice(a2, [32, 32], [0 * 32, 2 * 32])
        ...
        a_off3 = gl.slice(a_offsets, [32, BLOCK_K], [3 * 32, 0])
        a_inc3 = gl.slice(a_step, [32, BLOCK_K], [3 * 32, 0])
        a_off3 = a_off3 + a_inc3
        a_next_offsets = gl.slice_update(a_next_offsets, a_off3, [3 * 32, 0 * 32])
        a_next_ptrs3 = gl.slice(a_ptr + a_next_offsets, [32, BLOCK_K], [3 * 32, 0])
        a_next_mask3 = gl.slice(next_a_mask, [32, BLOCK_K], [3 * 32, 0])
        gl.metax.async_copy_global_to_shared(a_smem.index(3), a_next_ptrs3, mask=a_next_mask3, other=0.0,
)

        b_off3 = gl.slice(b_offsets, [32, BLOCK_N], [3 * 32, 0])
        b_inc3 = gl.slice(b_step, [32, BLOCK_N], [3 * 32, 0])
        b_off3 = b_off3 + b_inc3
        b_next_offsets = gl.slice_update(b_next_offsets, b_off3, [3 * 32, 0 * 32])
        b_next_ptrs3 = gl.slice(b_ptr + b_next_offsets, [32, BLOCK_N], [3 * 32, 0])

        a0k3 = gl.slice(a0, [32, 32], [0 * 32, 3 * 32])
        a3k0 = gl.slice(a3, [32, 32], [0 * 32, 0 * 32])
        c3 = gl.dot(a3k0, b0, c3, input_precision="tf32")
        b3 = gl.metax.bsm_perm(b3_raw)
        c0 = gl.dot(a0k3, b3, c0, input_precision="tf32")
        gl.metax.sched_bound()
        b0_next_raw = b_smem.index(0).load()
        a0_next = a_smem.index(0).load()
        a1k3 = gl.slice(a1, [32, 32], [0 * 32, 3 * 32])
        a3k1 = gl.slice(a3, [32, 32], [0 * 32, 1 * 32])
        c3 = gl.dot(a3k1, b1, c3, input_precision="tf32")
        c1 = gl.dot(a1k3, b3, c1, input_precision="tf32")
        gl.metax.iglp(config_2=2, config_5=1, config_6=5)
        b_next_mask3 = gl.slice(next_b_mask, [32, BLOCK_N], [3 * 32, 0])
        gl.metax.async_copy_global_to_shared(b_smem.index(3), b_next_ptrs3, mask=b_next_mask3, other=0.0,
)

        a3k2 = gl.slice(a3, [32, 32], [0 * 32, 2 * 32])
        c3 = gl.dot(a3k2, b2, c3, input_precision="tf32")
        gl.metax.gvm_arrive(8)
        gl.metax.barrier_shared()
        a1_next = a_smem.index(1).load()
        b1_next_raw = b_smem.index(1).load()
        b1_next = gl.metax.bsm_perm(b1_next_raw)
        a2k3 = gl.slice(a2, [32, 32], [0 * 32, 3 * 32])
        c2 = gl.dot(a2k3, b3, c2, input_precision="tf32")
        b0_next = gl.metax.bsm_perm(b0_next_raw)
        a3k3 = gl.slice(a3, [32, 32], [0 * 32, 3 * 32])
        c3 = gl.dot(a3k3, b3, c3, input_precision="tf32")
        gl.metax.iglp(config_2=2, config_5=1, config_6=5)

        acc = gl.slice_update(acc, c0, [0 * 32, 0 * 32])
        acc = gl.slice_update(acc, c1, [1 * 32, 0 * 32])
        acc = gl.slice_update(acc, c2, [2 * 32, 0 * 32])
        acc = gl.slice_update(acc, c3, [3 * 32, 0 * 32])
