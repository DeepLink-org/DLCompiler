...
M128 implementation.  Explicit storage aliases reuse one physical Shared
backing across the following ordered lifetimes::

    prologue:  sQ[M, 128]
    main loop: sK[128, 64], then sV[64, 128]
    epilogue:  sO[M, 128]
...
def _flash_fwd_tile(
    ...
    k0_smem = k_fragments.index(0)
    k1_smem = k_fragments.index(1)
    k0_smem.store(k0_register.permute(1, 0))
    k1_smem.store(k1_register.permute(1, 0))
    current_v0_ptrs = (
        v_head
        + n0[:, None] * v_stride_n
    ...
    if MASKED:
        ...
        v0_register = gl.load(current_v0_ptrs)
        v1_register = gl.load(current_v1_ptrs)
    ...
    # K and V are independent logical roots on the same physical backing.
    # Complete every K read before any warp starts the V lifetime.
    gl.metax.barrier_shared()
    ...
    v_fragments.index(0).store(v0_register)
    v_fragments.index(1).store(v1_register)

    has_next = n_block > 0
    next_n_block = max(n_block - 1, 0)
    next_n0 = next_n_block * BLOCK_N + gl.arange(0, 32)
    next_n1 = next_n_block * BLOCK_N + 32 + gl.arange(0, 32)
    next_valid0 = has_next & (next_n0[:, None] < SK)
    next_valid1 = has_next & (next_n1[:, None] < SK)
    next_k0_ptrs = (
        k_head
        + next_n0[:, None] * k_stride_n
        + first_d[None, :] * k_stride_d
    )
    next_k1_ptrs = (
        k_head
        + next_n1[:, None] * k_stride_n
        + first_d[None, :] * k_stride_d
    )
    next_k0_register = gl.load(
        next_k0_ptrs, mask=next_valid0, other=0.0
    )
    next_k1_register = gl.load(
        next_k1_ptrs, mask=next_valid1, other=0.0
    )
    ...
    # The next iteration reuses the backing for K.  Do not let an early warp
    # overwrite V while another warp still consumes it.
    gl.metax.barrier_shared()
    return next_k0_register, next_k1_register, acc_o, m_i, l_i
...
def flash_fwd_hdim128_generic_kernel(
    ...
    # Match TLX storage reuse explicitly. Each logical allocation is an
    # independent Shared-layout root, while storage-alias lowering gives all
    # of them one zero-copy physical backing. The program retains responsibility
    # for the non-overlapping Q, K, V, and O lifetimes and barriers below.
    ...
    first_n0 = first_n_block * BLOCK_N + gl.arange(0, 32)
    first_n1 = first_n_block * BLOCK_N + 32 + gl.arange(0, 32)
    first_d = gl.arange(0, BLOCK_D)
    first_k0_ptrs = (
        k_head
        + first_n0[:, None] * k_stride_n
        + first_d[None, :] * k_stride_d
    )
    first_k1_ptrs = (
        k_head
        + first_n1[:, None] * k_stride_n
        + first_d[None, :] * k_stride_d
    )
    if EVEN_MN and not CAUSAL:
        k0_register = gl.load(first_k0_ptrs)
        k1_register = gl.load(first_k1_ptrs)
    else:
        first_valid0 = (first_n_block >= 0) & (first_n0[:, None] < SK)
        first_valid1 = (first_n_block >= 0) & (first_n1[:, None] < SK)
        k0_register = gl.load(
            first_k0_ptrs, mask=first_valid0, other=0.0
        )
        k1_register = gl.load(
            first_k1_ptrs, mask=first_valid1, other=0.0
        )
    ...
    if BLOCK_M == 64 and CAUSAL and EVEN_MN:
        ...
        k0_register, k1_register, acc_o, m_i, l_i = _flash_fwd_tile(
            n_block,
            k0_register,
            k1_register,
            acc_o,
    ...
    gl.metax.barrier_shared()
    # Leave the register owner open. The single layout propagation pass derives
    # the D-contiguous global-store contract from this real memory access while
    # preserving the fixed Shared-memory reader map.
