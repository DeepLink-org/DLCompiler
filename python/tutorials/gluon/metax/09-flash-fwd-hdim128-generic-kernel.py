...
def _flash_fwd_tile(
    ...
    current_n = n_block * BLOCK_N + gl.arange(
        0,
        BLOCK_N,
        layout=gl.SliceLayout(1, global_layout),
    )
    qk_column_layout: gl.constexpr = gl.SliceLayout(0, qk_mma_layout)
    n0 = n_block * BLOCK_N + gl.arange(0, 32, layout=qk_column_layout)
    n1 = n_block * BLOCK_N + 32 + gl.arange(
        0, 32, layout=qk_column_layout
    )
    ...
    if BLOCK_M == 64:
        ...
        k0_smem.store(
            gl.slice(k_register, [32, BLOCK_D], [0, 0]).permute(1, 0)
        )
        k1_smem.store(
            gl.slice(k_register, [32, BLOCK_D], [32, 0]).permute(1, 0)
        )
    else:
    ...
    kt0_for_score = k0_smem.load(
        layout=qk_b_layout,
    )
    score0 = gl.dot(
        q_for_score,
        kt0_for_score,
        gl.zeros((BLOCK_M, 32), gl.float32, layout=qk_mma_layout),
        input_precision="tf32",
    )
    kt1_for_score = k1_smem.load(
        layout=qk_b_layout,
    )
    score1 = gl.dot(
        q_for_score,
    ...
    v_fragments.index(0).store(
        gl.slice(v_register, [32, BLOCK_D], [0, 0])
    )
    v_fragments.index(1).store(
        gl.slice(v_register, [32, BLOCK_D], [32, 0])
    )

    has_next = n_block > 0
    next_n_block = max(n_block - 1, 0)
    next_n = next_n_block * BLOCK_N + gl.arange(
        0,
        BLOCK_N,
        layout=gl.SliceLayout(1, global_layout),
    )
    ...
    alpha_for_o = gl.convert_layout(alpha[:, None], pv_mma_layout)
    acc_o = acc_o * alpha_for_o
    m_i = gl.where(row_has_key, m_ij, m_i)

    # QK-C and PV-A have identical ownership under the current [4, 1]
    # layouts, so these are register retags rather than shared exchanges.
    p0 = gl.convert_layout(
        p0_qk.to(v_head.dtype.element_ty),
        pv_a_layout,
        assert_trivial=True,
    )
    p1 = gl.convert_layout(
        p1_qk.to(v_head.dtype.element_ty),
        pv_a_layout,
        assert_trivial=True,
    )
    v00 = v_fragments.index(0).load(
        layout=pv_b_layout,
    )
    acc_o = gl.dot(p0, v00, acc_o, input_precision="tf32")

    v10 = v_fragments.index(1).load(
        layout=pv_b_layout,
    )
...
def flash_fwd_hdim128_generic_kernel(
    ...
    qk_mma_layout: gl.constexpr = gl.MACAMmaLayout(
        version_major=2,
        version_minor=0,
        # Four warps partition the selected M tile without duplicating either
        # N32 score fragment.  PV retains the same ownership.
        warps_per_cta=[4, 1],
        elements_mnk=qk_elements_mnk,
        # C's atom owns M by lane%16 and N by 4*(lane//16)+value, and lowers
        # the intrinsic as (B, A, C).  That is MACA col_major=1, not the c0
        # layout used by kernels that stage QK-C through shared before PV-A.
        col_major=1,
    )
    qk_a_layout: gl.constexpr = gl.DotOperandLayout(0, qk_mma_layout, 0)
    qk_b_layout: gl.constexpr = gl.DotOperandLayout(1, qk_mma_layout, 0)
    qk_row_layout: gl.constexpr = gl.SliceLayout(1, qk_mma_layout)
    pv_mma_layout: gl.constexpr = gl.MACAMmaLayout(
        version_major=2,
        version_minor=0,
        # Match QK-C ownership so each score fragment becomes PV-A by a
        # trivial register retag.  V uses the ordinary Dot-B load contract.
        warps_per_cta=[4, 1],
        elements_mnk=pv_elements_mnk,
        # Keep PV-C in the same C-aligned accumulator orientation as QK-C.
        col_major=1,
    )
    pv_a_layout: gl.constexpr = gl.DotOperandLayout(0, pv_mma_layout, 0)
    pv_b_layout: gl.constexpr = gl.DotOperandLayout(1, pv_mma_layout, 0)
    # C's B128 tiled copy uses eight lanes per row and eight contiguous half
    # values per lane; four warps then cover the 32-row copy atom.
    ...
    # Each logical Shared object retains its explicit layout while sharing one
    # 32 KiB physical backing. Their non-overlapping lifetimes are synchronized
    # by the barriers below.
    shared_storage = gl.storage_alias_spec(storage=gl.storage_kind.smem)
    if BLOCK_M == 64:
        qo_d64_pages = gl.local_alloc(
            q_ptr.dtype.element_ty,
            [BLOCK_M, BLOCK_D // 64, 64],
            layout=qo_d64_page_layout,
            reuse=shared_storage,
        ).index(0)
        q_smem = qo_d64_pages.reshape([BLOCK_M, BLOCK_D])
        o_d64_pages = gl.local_alloc(
            o_ptr.dtype.element_ty,
            [BLOCK_M, BLOCK_D // 64, 64],
            layout=qo_d64_page_layout,
            reuse=shared_storage,
        ).index(0)
        o_smem = o_d64_pages.reshape([BLOCK_M, BLOCK_D])
    else:
        q_smem = gl.local_alloc(
            q_ptr.dtype.element_ty,
            [BLOCK_M, BLOCK_D],
            layout=qkv_layout,
            reuse=shared_storage,
        ).index(0)
        o_smem = gl.local_alloc(
            o_ptr.dtype.element_ty,
            [BLOCK_M, BLOCK_D],
            layout=qkv_layout,
            reuse=shared_storage,
        ).index(0)
    k_tn_smem = gl.local_alloc(
        k_ptr.dtype.element_ty,
        [BLOCK_D, BLOCK_N],
        layout=tn_layout,
        reuse=shared_storage,
    ).index(0)
    v_fragments = gl.local_alloc(
        v_ptr.dtype.element_ty,
        [32, BLOCK_D],
        num_buffers=BLOCK_N // 32,
        layout=v_layout,
        reuse=shared_storage,
    )
    # Q and O are both D-contiguous in global memory.  Keep this producer /
    # sink layout explicit because layout propagation currently merges the Q
    # and O register constraints across their lifetime-disjoint shared alias.
    q_load_m = pid_m * BLOCK_M + gl.arange(
        0,
        BLOCK_M,
        layout=gl.SliceLayout(1, global_layout),
    )
    q_load_d = gl.arange(
        0,
        BLOCK_D,
        layout=gl.SliceLayout(0, global_layout),
    )
    q_ptrs = (
        q_head
    ...
    q_m = pid_m * BLOCK_M + gl.arange(
        0,
        BLOCK_M,
        layout=qk_row_layout,
    )
    ...
    first_n = first_n_block * BLOCK_N + gl.arange(
        0,
        BLOCK_N,
        layout=gl.SliceLayout(1, global_layout),
    )
    ...
    row_has_key_for_o = gl.convert_layout(
        row_has_key[:, None],
        pv_mma_layout,
    )
    safe_l_for_o = gl.convert_layout(safe_l[:, None], pv_mma_layout)
    out = gl.where(
        row_has_key_for_o,
        acc_o / safe_l_for_o,
        0.0,
...
def _run_case(seq_len, dtype, causal):
    torch.manual_seed(0)
...
def main():
    ...
    for seq_len in tuple(dict.fromkeys(args.seq_len)):
        result = _run_case(seq_len, dtype, args.causal)
