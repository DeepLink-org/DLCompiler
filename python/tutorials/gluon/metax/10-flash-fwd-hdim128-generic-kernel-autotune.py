"""Layout-autotuned C500 D128 M64/M128 FlashAttention forward.

The algorithm, global-memory accesses, prefetch order, Shared-memory lifetimes,
and synchronization order follow the explicit-layout tutorial. This clone
leaves all layouts to the compiler and contains no source-level layout
conversion. For better physical Shared access, both tile shapes stage K as two
N32 fragments instead of preserving the M128 whole-tile store.

``SQ`` and ``SK`` do not specialize the kernel binary. Layout winners dispatch
on their independent 128x ranges.
"""
...
def _flash_fwd_tile(
    ...
    k0_smem = k_storage.index(0)
    k1_smem = k_storage.index(1)
    k0_smem.store(
