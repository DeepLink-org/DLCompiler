...
def flash_bwd_fused_kernel(
    ...
    # The score/dP/dQ family retains the C kernel's 2x4 wave ownership.  For
    # dK/dV, the C 4x2 layout is a useful conformance reference but is about 3x
    # slower with this Gluon lowering.  The explicit 2x4/e228/col-major layout
    # below is the measured C500 performance variant; it keeps the C pipeline
    # while avoiding the slow dKV fragment contract.
    sdp_mma_layout: gl.constexpr = gl.MACAMmaLayout(
        version_major=2,
        version_minor=0,
        warps_per_cta=[2, 4],
        elements_mnk=[1, 1, 8],
    ...
    sdp_a_layout: gl.constexpr = gl.DotOperandLayout(
        0, sdp_mma_layout, 0
    )
    sdp_b_layout: gl.constexpr = gl.DotOperandLayout(
        1, sdp_mma_layout, 0
    )
    sdp_row_layout: gl.constexpr = gl.SliceLayout(1, sdp_mma_layout)
    sdp_col_layout: gl.constexpr = gl.SliceLayout(0, sdp_mma_layout)
    dkv_mma_layout: gl.constexpr = gl.MACAMmaLayout(
        version_major=2,
        version_minor=0,
        warps_per_cta=[2, 4],
        elements_mnk=[2, 2, 8],
    ...
    dkv_a_layout: gl.constexpr = gl.DotOperandLayout(
        0, dkv_mma_layout, 0
    )
    dkv_b_layout: gl.constexpr = gl.DotOperandLayout(
        1, dkv_mma_layout, 0
    )

    # These producers/sinks make every register layout in the C-style dataflow
    # explicit.  The row-major layout is also the proven C500 B128 async-copy
    # carrier in the current lowering; translating the CUTE 2x8 thread shape
    # literally produces wrong data because CUTE and TTG order conventions
    # differ.
    kv_nd_global_layout: gl.constexpr = gl.BlockedLayout(
        [4, 4], [4, 16], [8, 1], [1, 0]
    )
    kv_dn_global_layout: gl.constexpr = gl.BlockedLayout(
        [4, 4], [16, 4], [1, 8], [0, 1]
    )
    qdo_global_layout: gl.constexpr = gl.BlockedLayout(
        [1, 8], [4, 16], [8, 1], [1, 0]
    )
    raw_stage_layout: gl.constexpr = gl.BlockedLayout(
        [1, 1], [2, 32], [2, 4], [1, 0]
    )

    # Each logical Shared root keeps its explicit layout while sharing one
    # storage-alias class. The selected buffer index preserves the C kernel's
    # K/V halves and reverse-M epoch slots.
    shared_storage = gl.storage_alias_spec(storage=gl.storage_kind.smem)
    kv_tt_shared_layout: gl.constexpr = gl.SwizzledSharedLayout(
        vec=16, per_phase=8, max_phase=4, order=[1, 0]
    )
    ...
    ds_transposed_epoch = gl.local_alloc(
        ...
        [BLOCK_N, BLOCK_M],
        num_buffers=7,
        layout=transposed_stage_layout,
        reuse=shared_storage,
    )
    ...
    v_storage = kv_tt_smem_alloc.index(1)
    ...
    for fragment in range(0, BLOCK_N // N_FRAGMENT):
        vt_d = gl.arange(
            0, BLOCK_D, layout=gl.SliceLayout(1, kv_dn_global_layout)
        )
        vt_n = (
            pid_n * BLOCK_N
            + fragment * N_FRAGMENT
            + gl.arange(
                0,
                N_FRAGMENT,
                layout=gl.SliceLayout(0, kv_dn_global_layout),
            )
        )
        ...
        k_n = (
            ...
            + gl.arange(
                0,
                N_FRAGMENT,
                layout=gl.SliceLayout(1, kv_nd_global_layout),
            )
        )
        k_d = gl.arange(
            0, BLOCK_D, layout=gl.SliceLayout(0, kv_nd_global_layout)
        )
        k_ptrs = (
            k_head
            + k_n[:, None] * k_stride_n
        ...
        kt_for_score = k_tn_smem.load(layout=sdp_b_layout)
        vt_for_dp = v_tn_smem.load(layout=sdp_b_layout)
        gl.metax.barrier_shared()

        k_tt_smem.store(k_register)
        gl.metax.barrier_shared()
        k_for_dq = k_tt_smem.load(layout=sdp_b_layout)
        # Both K/V consumer fragments are resident before the C shared union
        # is reused by the initial Q and dO async copies.
        gl.metax.barrier_shared()

        acc_dk = gl.zeros(
            (N_FRAGMENT, BLOCK_D), gl.float32, layout=dkv_mma_layout
        )
        acc_dv = gl.zeros(
            (N_FRAGMENT, BLOCK_D), gl.float32, layout=dkv_mma_layout
        )
        ...
        initial_m = initial_m_block * BLOCK_M + gl.arange(
            0, BLOCK_M, layout=gl.SliceLayout(1, qdo_global_layout)
        )
        initial_d = gl.arange(
            0, BLOCK_D, layout=gl.SliceLayout(0, qdo_global_layout)
        )
        initial_q_ptrs = (
            q_head
        ...
        for m_iteration in range(0, num_m_blocks):
            ...
            q_stage = q_raw_smem.load(layout=raw_stage_layout)
            q_smem.store(q_stage)
            gl.metax.barrier_shared()

            # Reconstruct P first so score and dP can reuse one fp32 boundary.
            q_score = q_smem.load(layout=sdp_a_layout)
            score = gl.dot(
                q_score,
                kt_for_score,
                gl.zeros(
            ...
            next_q_m = (m_block - 1) * BLOCK_M + gl.arange(
                0, BLOCK_M, layout=gl.SliceLayout(1, qdo_global_layout)
            )
            next_q_d = gl.arange(
                0, BLOCK_D, layout=gl.SliceLayout(0, qdo_global_layout)
            ...
            lse_m = m_block * BLOCK_M + gl.arange(
                0, BLOCK_M, layout=sdp_row_layout
            )
            lse = gl.load(
            ...
            dpsum_m = m_block * BLOCK_M + gl.arange(
                0, BLOCK_M, layout=sdp_row_layout
            )
            dpsum = gl.load(
            ...
            do_stage = do_raw_smem.load(layout=raw_stage_layout)
            ...
            point_m = m_block * BLOCK_M + gl.arange(
                0, BLOCK_M, layout=sdp_row_layout
            )
            point_n = (
                pid_n * BLOCK_N
                + fragment * N_FRAGMENT
                + gl.arange(0, N_FRAGMENT, layout=sdp_col_layout)
            )
            ...
            do_for_dp = do_smem.load(layout=sdp_a_layout)
            dp = gl.dot(
                do_for_dp,
                vt_for_dp,
                gl.zeros(
            ...
            pt = p_transposed_smem.load(layout=dkv_a_layout)
            do_for_dv_raw = do_raw_smem.load(layout=dkv_b_layout)
            ds_element = (
                p * (dp - dpsum[:, None])
            ...
            gl.metax.barrier_shared()
            if m_block > 0:
                next_do_m = (m_block - 1) * BLOCK_M + gl.arange(
                    0,
                    BLOCK_M,
                    layout=gl.SliceLayout(1, qdo_global_layout),
                )
                next_do_d = gl.arange(
                    0,
                    BLOCK_D,
                    layout=gl.SliceLayout(0, qdo_global_layout),
                )
                next_do_ptrs = (
                    do_head
            ...
            ds_for_dq = ds_direct_smem.load(layout=sdp_a_layout)
            dq = gl.zeros(
                (BLOCK_M, BLOCK_D),
                gl.float32,
                layout=sdp_mma_layout,
            ...
            dq_m = m_block * BLOCK_M + gl.arange(
                0,
                BLOCK_M,
                layout=sdp_row_layout,
            )
            dq_d = gl.arange(
                0,
                BLOCK_D,
                layout=sdp_col_layout,
            )
            dq_ptrs = (
                dq_head
            ...
            q_for_dk_raw = q_raw_smem.load(layout=dkv_b_layout)
            gl.atomic_add(
                dq_ptrs,
                dq_scaled,
                sem="relaxed",
            ...
            # the dK MMA.
            dst = ds_transposed_smem.load(layout=dkv_a_layout)
        ...
        dk_out = k_tt_smem.load(layout=kv_nd_global_layout)
        ...
        dv_out = v_storage.load(layout=kv_nd_global_layout)
