"""Layout-autotuned single-kernel FlashAttention backward for MetaX C500.
...
Q/dO retain the C500 GVM/shared synchronization pipeline. No global P, dP, or
dS workspace is materialized. This clone leaves register, Shared, dot-operand,
and MMA layouts to the compiler's native post-TTGIR layout autotuner.
...
def flash_bwd_fused_kernel(
    ...
    # Keep the C kernel's storage arena and lifetimes, but leave every logical
    # Shared root layout open for compiler selection.
    ...
    for fragment in range(0, BLOCK_N // N_FRAGMENT):
        ...
        for m_iteration in range(0, num_m_blocks):
            ...
            score = gl.dot(
                q_score,
                kt_for_score,
                gl.zeros((BLOCK_M, N_FRAGMENT), gl.float32),
            ...
            gl.metax.barrier_shared()
            if m_block > 0:
                next_do_m = (m_block - 1) * BLOCK_M + gl.arange(0, BLOCK_M)
            ...
            dq = gl.zeros((BLOCK_M, BLOCK_D), gl.float32)
            dq = gl.dot(ds_for_dq, k_for_dq, dq, input_precision="tf32")
            dq_m = m_block * BLOCK_M + gl.arange(0, BLOCK_M)
            dq_d = gl.arange(0, BLOCK_D)
            dq_ptrs = (
                dq_head
...
def _measure_case(seq_len, dtype, warmup, rep, profile_rep):
    ...
    # Native layout autotuning benchmarks several compiled candidates. Its
    # first call therefore uses disposable destructive backward outputs; the
    # selected winner then runs once on fresh outputs for correctness.
