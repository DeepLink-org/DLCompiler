...
from python.build_helpers import (get_base_dir, get_cmake_dir, get_dist_dir, get_pymaca_project, get_pymaca_cmake_dir,
                                  get_pymxshmem_cmake_dir)
...
class CMakeBuild(build_ext):
    ...
    def build_pymxshmem_project(self, ext, project_dir):
        ...
        if not os.path.exists(project_dir):
            raise RuntimeError(f"{project_name} source directory not found")
        try:
            out = subprocess.check_output(["cmake_maca", "--version"])
        except OSError:
        ...
        # set mxshmem path
        # mxshmem_home = os.environ.get('MACA_PATH')
        # cmake_args += [f"-DMXSHMEM_INCLUDE_DIRS={mxshmem_home}/include/mxshmem",
        #                f"-DMXSHMEM_LIBRARY_DIRS={mxshmem_home}/lib/"]
        triton_dir = get_base_dir()
        cmake_args += [f"-DTRITON_SRC_DIR={triton_dir}"]
        # pybind_cmake_args = get_thirdparty_packages([get_pybind11_package_info()])
        cmake_args += self.get_pybind11_cmake_args()
        max_jobs = "8"
        build_args = ['-j' + max_jobs]
        env["PYTHONPATH"] = ""
        # Automatic GPU detection failed. Building for common architectures here
    ...
    def run(self):
        ...
        for ext in self.extensions:
            if ext.name == "triton" and check_env_flag("TRITON_BUILD_DISTRIBUTED", "OFF"):
                pymxshmem_ext_dir = os.path.join(get_dist_dir(), "shmem", "mxshmem_bind", "pymxshmem")
...
backends = [*BackendInstaller.copy(["nvidia", "amd", "metax"]), *BackendInstaller.copy_externals()]
...
def get_packages():
    ...
    # Add triton_dist packages when TRITON_BUILD_DISTRIBUTED is enabled
    if check_env_flag("TRITON_BUILD_DISTRIBUTED", "OFF"):
        # Add triton_dist itself and its subpackages
        triton_dist_dir = os.path.join(os.path.dirname(__file__), "third_party", "triton_dist", "python", "triton_dist")
        if os.path.isdir(triton_dist_dir):
            yield "triton_dist"
            # Add language subpackages
            triton_dist_language_dir = os.path.join(triton_dist_dir, "language")
            if os.path.isdir(triton_dist_language_dir):
                yield "triton_dist.language"

            # Add other triton_dist subpackages
            for subpkg in [
                    "kernels",
                    "tools",
                    "language/extra",
                    #"layers", "mega_triton_kernel", "models", "test"
            ]:
                subdir = os.path.join(triton_dist_dir, subpkg)
                if os.path.isdir(subdir):
                    subpkg_name = '.'.join(subpkg.split("/"))
                    yield f"triton_dist.{subpkg_name}"
                    for y in os.listdir(subdir):
                        nested = os.path.join(subdir, y)
                        if os.path.isdir(nested):
                            yield f"triton_dist.{subpkg_name}.{y}"
...
def add_link_to_triton_dist():
    triton_dist_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "third_party", "triton_dist", "python", "triton_dist"))
    triton_dist_install_dir = os.path.join(os.path.dirname(__file__), "python", "triton_dist")
    update_symlink(triton_dist_install_dir, triton_dist_dir)


def add_link_to_pymxshmem():
    pymxshmem_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "third_party", "triton_dist", "shmem", "mxshmem_bind", "pymxshmem",
                     "python", "pymxshmem"))
    pymxshmem_install_dir = os.path.join(os.path.dirname(__file__), "python", "triton", "pymxshmem")
...
def get_git_version_suffix():
    ...
    if branch.startswith("release"):
        return ""
    elif os.getenv("TRITON_BUILD_MACA_VERSION"):
        return "+metax" + os.getenv("TRITON_BUILD_MACA_VERSION")
    else:
