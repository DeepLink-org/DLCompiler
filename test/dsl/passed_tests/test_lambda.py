...
def fn_broadcast(output_ptr, x_ptr, length):
    ...
    result = (
        input.reshape((XS, 1, ZS))
        .broadcast_to((XS, YS, ZS))
        .reshape((count(XS, YS, ZS)))
    )
    brc_col_offsets = tl.arange(0, NUMEL * YS)
    tl.store(output_ptr + brc_col_offsets, result)


@pytest.mark.parametrize("nblocks", [NBLOCKS])
def test_broadcast(nblocks):
    length = NUMEL
...
@pytest.mark.parametrize("input_shape,block_size", [((128, 128), 16)])
def test_custom_copy(input_shape, block_size):
    # 创建输入张量 (128x128) 和输出张量 (16x16)
    input_tensor = torch.randn(*input_shape, device="npu")
    output_tensor = torch.zeros(block_size, block_size, device="npu")

    grid = (1, 1)
    custom_copy_kernel[grid](
        input_tensor,
        output_tensor,
        input_row_size=input_shape[1],
        block_size=block_size,
    )

    expected = input_tensor[::8, ::8][:block_size, :block_size]
    # print(f"Expected shape: {expected}, Output shape: {output_tensor}")
    assert torch.allclose(
        output_tensor, expected, atol=1e-6
