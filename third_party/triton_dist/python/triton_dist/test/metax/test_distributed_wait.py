...
import triton_dist.language.extra.maca.language_extra as language_extra
...
def kernel_consumer_gemm(
    ...
    if needs_wait:
        ...
        # token = language_extra.ld_acquire(ready_ptr, "gpu")
        # while token != 1:
        #     token = language_extra.ld_acquire(ready_ptr, "gpu")
        # a_ptrs = dl.consume_token(a_ptrs, token)
...
def test_1024_gemm_single_device(args):
    ...
    ag_A = torch.randn([M, K], dtype=dtype, device=device)
    B = torch.randn([K, N_per_rank], dtype=dtype, device=device)
    C = torch.randn([M, N_per_rank], dtype=dtype, device=device)

    C_golden = torch.matmul(ag_A, B)
    C_origin = torch.clone(C)
    ...
    assert torch.allclose(C_origin, C, atol=1e-3, rtol=1e-3)
