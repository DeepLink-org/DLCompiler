...
void init_triton_dicp_triton_pass_linked_npu(py::module &&m) {
  ...
  m.def("add_npu_unroll_pipeline", [](mlir::PassManager &pm) {
    pm.addNestedPass<mlir::func::FuncOp>(
        dicp::LinalgExt::createPipelineLoopUnrollPass());
  });

  m.def("add_npu_vector_tile_pipeline",
        [](mlir::PassManager &pm, unsigned vectorTile, bool tileAllBlocks) {
          mlir::dicp::LinalgExt::VectorTilingPipelineOptions opts;
          opts.vectorTile = vectorTile;
          opts.tileAllBlocks = tileAllBlocks;
          pm.addPass(
              mlir::dicp::LinalgExt::createVectorTilingPipelinePass(opts));
        });
